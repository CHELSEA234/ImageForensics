...
def get_dataloader(
    ...
    # print("...inside the dataloader function...")
    # print(len(real_img_list))
    # print("...inside the dataloader function...")
    # import sys;sys.exit(0)

    # label_list = ground_truth_L1 # you can decide what to use here.
    ## GX: add different folder into the list:
    ## GX: add different samples from each folder into the list:
    ...
    for idx, folder_name in enumerate(folder_lst):
        ...
        elif mode != 'train' and folder_name_ in set_val:
            cls0_sam_list += cur_lst
...
def get_inference_dataloader(
    ...
    redDataset = RedInferenceDataset(img_path_lst, 
                                    manipulations_dict, 
                                    transform=transform)
    redGenerator = data.DataLoader(redDataset,**params)
    return redGenerator, redDataset
...
class RealsampleDataset(data.Dataset):
    def __init__(self, img_list, transform):
        ...
        self.label0 = np.array([0] * 43)  # dummy red label
        self.label1 = 0
...
class RedInferenceDataset(RedDataset):
    def __getitem__(self, index):
        image_name = random.choice(self.img_list)
        image    = imageio.imread(image_name, pilmode='RGB')
        if image.shape[-1] == 4:
            assert ValueError, print(f"The image is not the RGB format.")
        rgb_preproc = self.transform(image)
        self.label0 = np.array([1] * 43)  # dummy red label
        self.label1 = 0
        return rgb_preproc, self.label0, self.label1, image_name
