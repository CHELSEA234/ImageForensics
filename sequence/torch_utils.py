...
def inference(model,val_generator,criterion,device,adj,task_num=29,desc='valid',debug=False):
    '''
        the inference code.
    '''
    ...
    for jb, val_batch in enumerate(val_generator):
        ...
        adj = torch.unsqueeze(adj[0,:,:], axis=0)
...
def infererence_interporator(pred_list):
    '''
        interpret result to text.
    '''
    groud_truth_lst = ['L1', "L2", "MSE", "MMD", "LS", "WGAN", "KL", "Adv.", "Hinge", "CE",
                        "ReLU", "Tanh", "L_ReLU", "Sig", "SiLU", 
                        "ELU", "ReLU", "L_ReLU", "Sig", "SiLU", 
                        "Up_sampling", "Deconv", "Skip", "Down_sampling"
                        ]
    answer_lst = []
    for _ in range(len(groud_truth_lst)):
        if pred_list[_][0] == 1:
            answer_lst.append(groud_truth_lst[_])

    return answer_lst

def format_str(input_list):
    # return "%.3f" % value
    # return ["%.3f" % value for value in input_list]
    res = ''
...
def dump_to_txt_helper(args, res_list):
    ...
    args.txt_handler.write(obj_lst+'\n')
    args.txt_handler.write(con_arc+'\n')
    args.txt_handler.write(dis_arc+'\n')
    args.txt_handler.write(res_str+'\n')
...
def save_inter_graph(args, epoch, pred_fea, gcn_out, image_name, 
    ...
    # # print(mm_1.shape)       # (14, 31)
    # # print(mm_2.shape)       # (14, 31)
    # # print(cg_1.shape)       # (14, 31)
    # # print(cg_2.shape)       # (31, 14)
    # print(adj_0.shape)      # (31, 31)
    # # print(adj_1.shape)      # (14, 14)
    # print(adj_2.shape)
    # print()
    # print(adj_2)
    # # adj_sum = adj_0 + adj_2
    ...
    # if debug:
    #     args.att_graph_folder = args.att_graph_folder + '_debug'
...
def display_red_eval_tb(writer,macro_f1,micro_f1,accu,val_loss_avg,tot_iter,desc='valid'):
    ## GX: why in your case, micro_f1 == accu?
...
def train_logging(
    ...
    #     writer.add_scalar('progress/accu', accu, tot_iter)
